	...
	*ptr_frame_info = LIST_FIRST(&free_frame_list);
	...
	if (*ptr_frame_info == NULL)
	{
		...
		if(queue_size(&env_exit_queue)>0){
			...
			struct Env *itr_exit=NULL;
			LIST_FOREACH(itr_exit,&(env_exit_queue)){
				if(itr_exit->env_status==ENV_EXIT)
				{
					env_free(itr_exit);
				}
			}
			...
			env_page_ws_invalidate(curenv,curenv->SecondList.lh_first->virtual_address);
		}
	}

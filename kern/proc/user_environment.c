...
//	return;
	...
	// your code is here, remove the panic and write your code
	{
		...
		struct WorkingSetElement *ptr=e->SecondList.lh_first;
		while (e->SecondList.lh_first!=NULL)
		{
			unmap_frame(e->env_page_directory, ptr->virtual_address);
			ptr= e->SecondList.lh_first;
			LIST_REMOVE(&(e->SecondList),ptr);
			kfree(ptr);
		}


		for (uint32 i =USER_HEAP_START; i<USER_TOP ;i+=PAGE_SIZE*NPDENTRIES)
		{
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart, uint32 daLimit)
{
	...
	e->segment_break=e->start;
	e->UserHardLimit=ROUNDUP(daLimit, PAGE_SIZE);


	initialize_dynamic_allocator(e->start,initSizeOfAllocatedSpace);

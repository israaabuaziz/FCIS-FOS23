...
int process_command(int number_of_arguments, char** arguments)
{
	...
	//Check for CMD_VALID & CMD_INV_NUMARGS
	int INV_NUM_ARGSNumber = 0;
	for(int i=0 ; i<NUM_OF_COMMANDS ; i++)
	{
		if(strcmp(arguments[0],commands[i].name) == 0)
		{
			...
			else if(commands[i].num_of_args == number_of_arguments-1)
			{
				return i;
			}
			else
			{
				if(INV_NUM_ARGSNumber == 0)
	...
	// CMD_MATCHED& CMD_INVALIED
	     int subsequence_num=0;
		 int subsequence_found ;
		for(int i = 0; i<NUM_OF_COMMANDS; i++)
	 {
			subsequence_found = 0;
		...
		if (strcmp(arguments[0],commands[i].name) != 0)
		 {
			...
			int last_index_matched = -1;
	        int entered_command_Length = strlen(arguments[0]);
	        int known_command_Length = strlen(commands[i].name);

	        for (int j = 0; j < entered_command_Length; j++)
	        {
	        	for(int z=0 ; z < known_command_Length ; z++ )
	        	{
	        		if (arguments[0][j] == commands[i].name[z] && z > last_index_matched )
	        	  {
	        		  last_index_matched = z;
	        		  subsequence_found++;
	        ...
	        if(subsequence_found == entered_command_Length)
	        {
	        	LIST_INSERT_HEAD(&foundCommands, &commands[i]);
	        	subsequence_num++;
	        }

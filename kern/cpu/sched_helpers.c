...
void env_set_nice(struct Env* e, int nice_value)
{
	...
	{
		fixed_point_t updated_priority = fix_sub( fix_sub( fix_int((int)PRI_MAX) , fix_unscale(fix_int(e->env_recent_cpu),4) ), fix_scale(fix_int(e->env_nice),2) );
		int res = fix_round(updated_priority);// to be in range PRI_MIN to PRI_MAX.

		if(res > (num_of_ready_queues - 1))
			res = (num_of_ready_queues - 1);
		else if(res < PRI_MIN)
			res = PRI_MIN;
...
int env_get_recent_cpu(struct Env* e)
{
	...
	fixed_point_t a = fix_scale(fix_int(e->env_recent_cpu),100);
	int res = fix_round(a);
...
int get_load_average()
{
	...
	fixed_point_t a = fix_scale(fix_int(load_average),100);
	int res = fix_round(a);

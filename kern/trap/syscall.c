...
return curenv->UserHardLimit;
}

int sys_get_permissions(uint32 virtual_address)
{
...
{
	env_set_nice(curenv,nice_value); //env?
}
...
void* sys_sbrk(int increment)
{
	...
	uint32 hardlimit=env->UserHardLimit;
//	uint32 Sbrk = ROUNDUP(curenv->segment_break, PAGE_SIZE);
	uint32 Sbrk = env->segment_break;
	uint32 Previous = Sbrk;
//	cprintf("Incoming: %d\n", increment);

	if(increment == 0)
	{
//	    cprintf("in sbrk = 0 curenv->segment_break %x\n",env->segment_break);
//	    cprintf("in sbrk = 0 curenv->segment_break %x\n",env->segment_break + PAGE_SIZE);
	...
	{
//		   cprintf("in sbrk > 0\n");
		   uint32 New_Sbrk= ROUNDUP(Sbrk + (uint32)increment,PAGE_SIZE);

		   if(New_Sbrk>hardlimit)
		   {
//			   cprintf("Wrong 1\n");
			   return (void*)-1;
		   }
		   ...
		   {
			   uint32 size1 = ROUNDUP(increment,PAGE_SIZE);
			   allocate_user_mem(env, Previous, size1);

		   }

		env->segment_break=New_Sbrk;
//		cprintf("Old break: %x\n", Sbrk);
//		cprintf("New break: %x\n", env->segment_break);
	   return (void*) Sbrk;
	}
	...
	{
//			cprintf("in sbrk < 0\n");
		uint32 NegativeSbrk= Sbrk + (uint32)increment;

		if (NegativeSbrk < curenv->start)
		{
//			cprintf("Wrong 3\n");
		   return (void*)-1;
		}
		...
		{
			Sbrk = NegativeSbrk;
			curenv->segment_break = NegativeSbrk;
			 return (void*) Sbrk;
		}
		else if((increment*-1) >= 4)
		{
			int NumOfPages = (increment*-1)/PAGE_SIZE;
										    // < ?
			for (uint32 i = NegativeSbrk; i <= Previous ; i+=PAGE_SIZE)
			{
			 //un mark
			  pt_set_page_permissions(curenv->env_page_directory,i,0,PREM_MARK);
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_allocate_user_mem:
		if((unsigned int *)(a1+a2)==NULL || (a1+a2)>=USER_LIMIT ){
			sched_kill_env(curenv->env_id);
		}
		if((unsigned int *)(a1)==NULL || a1>=USER_LIMIT ){
					sched_kill_env(curenv->env_id);
				}
